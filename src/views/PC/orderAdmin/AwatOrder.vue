<template>
  <div class="awatOrder">
    ...
    <div class="dataContent">
      <el-table :data="allOrderList" v-loading="loading" height="620" border style="width: 100%">
        ...
        <el-table-column prop="des" label="操作" width="50">
          <template slot-scope="scope">
            <el-button @click="setOrderFn(scope.row.orderId)" type="text" size="small">操作</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    getOrderList() {
      this.axios.get('http://localhost:3005/allOrderList?_start=0&_end=10&rate=0')
        .then(res => {
          const { data } = res;
    ...
    setOrderFn(id) {
      console.log(id, "订单id");
      this.$confirm(`您确定要为订单${id}提交立即开始生产申请吗?`, '提示', {
